...
![An illustration of continually updating timeseries data where a
previous value unexpectedly
changes.](reference/figures/README-butterfly_diagram.png)
...
![An illustration of timeseries data not being continuous in the way it
is expected to be.](reference/figures/README-timeseries_dark.png)
...
Conversely,
[`butterfly::release()`](https://docs.ropensci.org/butterfly/reference/release.md)
drops all rows which had changed from the previous version. Note it
...
- [waldo](https://github.com/r-lib/waldo) - `butterfly` uses
  ...
  in every function to provide a report on difference. There is
  therefore significant overlap, however `butterfly` builds on `waldo`
  by providing the functionality of comparing objects where we expect
  some changes, with previous versions but not others. `butterfly` also
  provides extra user feedback to provide clarity on what it is and
...
- [diffdf](https://github.com/gowerc/diffdf) - similar to `waldo`, but
  ...
  [`butterfly::loupe()`](https://docs.ropensci.org/butterfly/reference/loupe.md)
  and `diffdf::diffdf_has_issues()` both provide a TRUE/FALSE difference
  check, while `diffdf::diffdf_issue_rows()` and
  [`butterfly::catch()`](https://docs.ropensci.org/butterfly/reference/catch.md)
  both return the rows where changes have occurred. However, it lacks
  the flexibility of `butterfly` to compare object where we expect some
...
[`all.equal()`](https://rdrr.io/r/base/all.equal.html) (base R) or
